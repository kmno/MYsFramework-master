...
class GDCalendar: UIView, UIGestureRecognizerDelegate{
    ...
    public func gotoNextMonth(){
        currentDate = self.currentDate.nextMonthGD
        setDates()
        generateDates()
...
class GDCalendarItemCell: UICollectionViewCell{
    ...
    private var sizeGD: CGFloat = 0.0
    ...
    private func generateHeaderLabel(){
        ...
        headerLabel.frame = CGRect(x: 0, y: 0, width: self.sizeGD, height: self.sizeGD)
        if day.count > 1{
    ...
    private func generateItemLabel(){
        ...
        itemLabel.sizeToFit()
        itemLabel.frame = CGRect(x: 0, y: 0, width: self.sizeGD, height: self.sizeGD)
    }
